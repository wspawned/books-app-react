    ...
    <Box sx={{ flexGrow: 1 }}>
      <AppBar position="static">
        <Toolbar>
            ...
            NAVBAR_ITEMS.map((item) => {
              return (
                <Typography variant="h6" component="div" sx={{ flexGrow: 1 }} >
                  <Link onClick={()=>{navigate(`${item.route}`)}} color="inherit" underline="hover" >
                    {`${item.name}`}
                  </Link>
                </Typography>
              )
          ...
          {activeUser.isAdmin &&                 
          <Typography variant="h6" component="div" sx={{ flexGrow: 1 }} >
              <Link onClick={()=>navigate("/admin")} color="inherit" underline="hover" >
                {`ADMIN PANEL`}
              </Link>
          </Typography>}
          ...
          {
            activeUser.username ?
            <Button onClick={()=>{dispatch(resetActiveUser())}} color="inherit">Logout</Button> :
